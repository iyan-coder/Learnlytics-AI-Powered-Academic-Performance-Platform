...
class DataValidation:
            ...
            for col in required_columns:
                if col not in dataframe.columns:
                    raise StudentPerformanceException(
                        f"Missing required column: {col}", sys
                    )
            ...
            for col, expected_dtype in expected_columns.items():
                if col in dataframe.columns:
                    ...
                    if expected_dtype != actual_dtype:
                        raise StudentPerformanceException(
                            f"Column '{col}' expected type {expected_dtype} but got {actual_dtype}",
                            sys,
                        )

            logger.info(
                "Schema column and data type validation completed successfully."
            )
            return True
        except Exception as e:
    ...
    def check_categorical_drift(
        self, base_df: pd.DataFrame, current_df: pd.DataFrame, schema: dict
    ) -> dict:
        """
        ...
        try:
            logger.info(
                "Checking for categorical feature drift using Chi-squared test..."
            )
            drift_report = {}
            ...
            for col in schema.get("categorical_columns", []):
                if col in base_df.columns and col in current_df.columns:
                    ...
                    categories = sorted(
                        set(base_counts.index).union(set(current_counts.index))
                    )
                    ...
                    if sum(base_freq) > 0 and sum(current_freq) > 0:
                        ...
                        logger.info(
                            f"Categorical drift check completed for column: {col} | p-value: {p_value:.4f}"
                        )
                    else:
                        logger.warning(
                            f"Skipped drift check for '{col}' due to insufficient data."
                        )
                else:
                    logger.warning(
                        f"Column '{col}' not found in both datasets. Skipping."
                    )
        ...
        try:
            ...
            for column in base_df.columns:
                if pd.api.types.is_numeric_dtype(base_df[column]):
                    ks_result = ks_2samp(
                        base_df[column].dropna(), current_df[column].dropna()
                    )
                    ...
                    logger.info(
                        f"Numerical drift check completed for column: {column} | p-value: {p_value:.4f}"
                    )
            ...
            categorical_drift = self.check_categorical_drift(
                base_df, current_df, self._schema_config
            )
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            if not self.validate_number_of_columns(
                train_df
            ) or not self.validate_schema_columns_and_types(train_df):
                raise StudentPerformanceException(
                    "Train dataframe schema invalid.", sys
                )
            if not self.validate_number_of_columns(
                test_df
            ) or not self.validate_schema_columns_and_types(test_df):
            ...
            if not self.is_numerical_columns_exist(train_df):
                raise StudentPerformanceException(
                    "Train dataframe lacks numerical columns.", sys
                )
            if not self.is_numerical_columns_exist(test_df):
                raise StudentPerformanceException(
                    "Test dataframe lacks numerical columns.", sys
                )
